...
function TestConsumer() {
  ...
  return (
    <div>
      ...
      <span data-testid="canUploadForAnyOrg">{String(ctx.canUploadForAnyOrg)}</span>
      ...
      <span data-testid="hasAdminPanel">{String(ctx.hasPermission('admin_panel'))}</span>
      <span data-testid="hasAllViewUpload">{String(ctx.hasAllPermissions(['view', 'upload']))}</span>
      <span data-testid="hasAnyDeleteClear">{String(ctx.hasAnyPermission(['delete', 'clear']))}</span>
      <span data-testid="isAtLeastViewer">{String(ctx.isAtLeast('viewer'))}</span>
      <span data-testid="isAtLeastManager">{String(ctx.isAtLeast('manager'))}</span>
...
function HasPermissionConsumer({ permission }: { permission: 'view' | 'upload' | 'export' | 'delete' | 'clear' | 'admin_panel' }) {
...
describe('PermissionContext', () => {
  ...
  describe('Manager Role', () => {
    ...
    it('should return false for hasAnyPermission with delete or clear', async () => {
      render(<TestConsumer />, { wrapper: createWrapper() });
      expect(screen.getByTestId('hasAnyDeleteClear')).toHaveTextContent('false');
    });
  });
  ...
  describe('Super Admin', () => {
    ...
    it('should have canUploadForAnyOrg=true', async () => {
      render(<TestConsumer />, { wrapper: createWrapper() });
      expect(screen.getByTestId('canUploadForAnyOrg')).toHaveTextContent('true');
    });
  });
  ...
  describe('Non-Super Admin', () => {
    ...
    it('should have canUploadForAnyOrg=false', async () => {
      render(<TestConsumer />, { wrapper: createWrapper() });
      expect(screen.getByTestId('canUploadForAnyOrg')).toHaveTextContent('false');
    });
  });
  ...
  describe('Denial Messages', () => {
    ...
    it('should return correct denial message for view', async () => {
      render(<TestConsumer />, { wrapper: createWrapper() });
      expect(screen.getByTestId('denialView')).toHaveTextContent('You do not have permission to view this content');
    });

    it('should return correct denial message for delete', async () => {
      render(<TestConsumer />, { wrapper: createWrapper() });
      expect(screen.getByTestId('denialDelete')).toHaveTextContent('Only Admins can delete transactions');
    });
  });
  ...
  describe('useHasPermission Hook', () => {
    ...
    it('should return true for allowed permission', async () => {
      render(<HasPermissionConsumer permission="view" />, { wrapper: createWrapper() });
      expect(screen.getByTestId('result')).toHaveTextContent('true');
    });

    it('should return false for disallowed permission', async () => {
      render(<HasPermissionConsumer permission="delete" />, { wrapper: createWrapper() });
