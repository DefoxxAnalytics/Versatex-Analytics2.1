...
import { OrganizationProvider, useOrganization, getOrganizationParam } from '../OrganizationContext';
...
function TestConsumer() {
  ...
  return (
    <div>
      <span data-testid="activeOrg">{ctx.activeOrganization?.name ?? 'null'}</span>
      <span data-testid="activeOrgId">{ctx.activeOrganization?.id ?? 'null'}</span>
      ...
      <span data-testid="isViewingOtherOrg">{String(ctx.isViewingOtherOrg)}</span>
      <span data-testid="isLoading">{String(ctx.isLoading)}</span>
      <button data-testid="switch-2" onClick={() => ctx.switchOrganization(2)}>Switch to Org 2</button>
      <button data-testid="reset" onClick={() => ctx.resetToDefault()}>Reset</button>
      <button data-testid="getRole-1" onClick={() => { document.getElementById('roleResult')!.textContent = ctx.getRoleInOrg(1) ?? 'null'; }}>Get Role 1</button>
...
const singleOrgUser = {
  ...
  profile: {
    ...
    organizations: [
      { organization: 1, organization_name: 'Org One', organization_slug: 'org-one', role: 'viewer' as const, is_primary: true },
    ],
  },
};
...
const multiOrgUser = {
  ...
  profile: {
    ...
    is_super_admin: false,
    organizations: [
      { organization: 1, organization_name: 'Org One', organization_slug: 'org-one', role: 'admin' as const, is_primary: true },
      { organization: 2, organization_name: 'Org Two', organization_slug: 'org-two', role: 'viewer' as const, is_primary: false },
      { organization: 3, organization_name: 'Org Three', organization_slug: 'org-three', role: 'manager' as const, is_primary: false },
    ],
  },
};
...
describe('OrganizationContext', () => {
  ...
  describe('Single-Org User', () => {
    ...
    it('should have isViewingOtherOrg=false when viewing primary', async () => {
      ...
      await waitFor(() => {
        expect(screen.getByTestId('isViewingOtherOrg')).toHaveTextContent('false');
      });
    });
  });
  ...
  describe('Organization Switching', () => {
    ...
    it('should set isViewingOtherOrg=true when viewing non-primary org', async () => {
      ...
      await waitFor(() => {
        expect(screen.getByTestId('isViewingOtherOrg')).toHaveTextContent('true');
      });
    });
