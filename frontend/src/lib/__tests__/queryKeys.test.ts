...
describe('Query Keys Factory', () => {
  ...
  describe('procurement', () => {
    ...
    describe('suppliers', () => {
      it('should have correct "all" key', () => {
        expect(queryKeys.procurement.suppliers.all).toEqual(['procurement', 'suppliers']);
      });
      ...
      it('should generate list key with params and orgId', () => {
        ...
        expect(key).toEqual(['procurement', 'suppliers', 'list', params, { orgId: 1 }]);
      });

      it('should generate detail key', () => {
        const key = queryKeys.procurement.suppliers.detail(42, 1);
        expect(key).toEqual(['procurement', 'suppliers', 'detail', 42, { orgId: 1 }]);
      });
    });

    describe('categories', () => {
      it('should have correct "all" key', () => {
        expect(queryKeys.procurement.categories.all).toEqual(['procurement', 'categories']);
      });

      it('should generate list key', () => {
        const key = queryKeys.procurement.categories.list({ is_active: true }, 1);
        expect(key).toEqual(['procurement', 'categories', 'list', { is_active: true }, { orgId: 1 }]);
      });

      it('should generate detail key', () => {
        const key = queryKeys.procurement.categories.detail(10, 2);
        expect(key).toEqual(['procurement', 'categories', 'detail', 10, { orgId: 2 }]);
      });
    });

    describe('transactions', () => {
      it('should have correct "all" key', () => {
        expect(queryKeys.procurement.transactions.all).toEqual(['procurement', 'transactions']);
      });
      ...
      it('should generate list key with filters', () => {
        ...
        expect(key).toEqual(['procurement', 'transactions', 'list', params, { orgId: 1 }]);
      });

      it('should generate detail key', () => {
        const key = queryKeys.procurement.transactions.detail(100, 1);
        expect(key).toEqual(['procurement', 'transactions', 'detail', 100, { orgId: 1 }]);
      });
    });

    describe('uploads', () => {
      it('should have correct "all" key', () => {
        expect(queryKeys.procurement.uploads.all).toEqual(['procurement', 'uploads']);
      });
      ...
      it('should generate detail key', () => {
        ...
        expect(key).toEqual(['procurement', 'uploads', 'detail', 5, { orgId: 1 }]);
      });
    });
  });
  ...
  describe('analytics', () => {
    ...
    describe('Pareto Analysis', () => {
      ...
      it('should generate paretoDrilldown key with supplierId', () => {
        ...
        expect(key).toEqual(['analytics', 'pareto-drilldown', 42, { orgId: 1 }]);
      });
    });
    ...
    describe('Seasonality', () => {
      ...
      it('should generate seasonalityDetailed key', () => {
        ...
        expect(key).toEqual(['analytics', 'seasonality-detailed', true, { orgId: 1 }]);
      });

      it('should generate seasonalityCategoryDrilldown key', () => {
        const key = queryKeys.analytics.seasonalityCategoryDrilldown(5, true, 1);
        expect(key).toEqual(['analytics', 'seasonality-category', 5, true, { orgId: 1 }]);
      });
    });
    ...
    describe('Year over Year', () => {
      it('should generate yearOverYear key with all params', () => {
        ...
        expect(key).toEqual(['analytics', 'yoy', true, 2023, 2024, { orgId: 1 }]);
      });

      it('should generate yoyDetailed key', () => {
        const key = queryKeys.analytics.yoyDetailed(false, 2022, 2023, 1);
        expect(key).toEqual(['analytics', 'yoy-detailed', false, 2022, 2023, { orgId: 1 }]);
      });

      it('should generate yoyCategoryDrilldown key', () => {
        const key = queryKeys.analytics.yoyCategoryDrilldown(3, true, 2023, 2024, 1);
        expect(key).toEqual(['analytics', 'yoy-category', 3, true, 2023, 2024, { orgId: 1 }]);
      });

      it('should generate yoySupplierDrilldown key', () => {
        const key = queryKeys.analytics.yoySupplierDrilldown(7, false, 2022, 2023, 1);
        expect(key).toEqual(['analytics', 'yoy-supplier', 7, false, 2022, 2023, { orgId: 1 }]);
      });
    });
  ...
  describe('p2p', () => {
    ...
    describe('Cycle Time', () => {
      ...
      it('should generate stageDrilldown key', () => {
        ...
        expect(key).toEqual(['p2p', 'stage-drilldown', 'pr_to_po', { orgId: 1 }]);
      });
    });
  ...
  describe('reports', () => {
    ...
    it('should generate detail key', () => {
      ...
      expect(key).toEqual(['reports', 'detail', 'report-uuid-123', { orgId: 1 }]);
    });

    it('should generate status key', () => {
      const key = queryKeys.reports.status('report-uuid-123', 1);
      expect(key).toEqual(['reports', 'status', 'report-uuid-123', { orgId: 1 }]);
    });
    ...
    it('should generate scheduleDetail key', () => {
      ...
      expect(key).toEqual(['reports', 'schedule-detail', 'schedule-uuid-456', { orgId: 1 }]);
    });
  });
  ...
  describe('Key Structure', () => {
    ...
    it('parameterized keys should include all parameters', () => {
      const key = queryKeys.analytics.yoyCategoryDrilldown(5, true, 2023, 2024, 1);
