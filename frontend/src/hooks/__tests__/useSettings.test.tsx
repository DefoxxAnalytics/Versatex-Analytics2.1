...
describe('useSettings Hooks', () => {
  ...
  describe('useUpdateSettings', () => {
    ...
    it('should sync to backend when authenticated', async () => {
      localStorage.setItem(USER_KEY, JSON.stringify({ id: 1 }));
      localStorage.setItem(SETTINGS_STORAGE_KEY, JSON.stringify(defaultSettings));
      vi.mocked(api.authAPI.updatePreferences).mockResolvedValue({ data: {} } as any);
    ...
    it('should not fail if backend sync fails', async () => {
      localStorage.setItem(USER_KEY, JSON.stringify({ id: 1 }));
      localStorage.setItem(SETTINGS_STORAGE_KEY, JSON.stringify(defaultSettings));
      vi.mocked(api.authAPI.updatePreferences).mockRejectedValue(new Error('Network error'));
  ...
  describe('useResetSettings', () => {
    ...
    it('should sync reset to backend when authenticated', async () => {
      localStorage.setItem(USER_KEY, JSON.stringify({ id: 1 }));
      localStorage.setItem(SETTINGS_STORAGE_KEY, JSON.stringify(customSettings));
      vi.mocked(api.authAPI.replacePreferences).mockResolvedValue({ data: {} } as any);
    ...
    it('should not fail if backend sync fails on reset', async () => {
      localStorage.setItem(USER_KEY, JSON.stringify({ id: 1 }));
      localStorage.setItem(SETTINGS_STORAGE_KEY, JSON.stringify(customSettings));
      vi.mocked(api.authAPI.replacePreferences).mockRejectedValue(new Error('Network error'));
