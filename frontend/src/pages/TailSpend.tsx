import { useDetailedTailSpend, useTailSpendCategoryDrilldown, useTailSpendVendorDrilldown } from '@/hooks/useAnalytics';
import { Card, CardContent, CardHeader, CardTitle } from '@/components/ui/card';
import { Button } from '@/components/ui/button';
import { Dialog, DialogContent, DialogHeader, DialogTitle } from '@/components/ui/dialog';
import { Input } from '@/components/ui/input';
import { Slider } from '@/components/ui/slider';
import { Package, BarChart as BarChartIcon, Info, Lightbulb, Target, DollarSign, Calendar, Layers, X, TrendingUp, MapPin, Loader2 } from 'lucide-react';
...
export default function TailSpend() {
  ...
  const { data: categoryDrilldown, isLoading: categoryLoading } = useTailSpendCategoryDrilldown(selectedCategoryId, threshold);
  const { data: vendorDrilldown, isLoading: vendorLoading } = useTailSpendVendorDrilldown(selectedVendorId, threshold);
  ...
  if (isLoading) {
    return (
      <div className="p-6 flex items-center justify-center min-h-[400px]">
        ...
        <span className="ml-2 text-muted-foreground">Loading tail spend analysis...</span>
      </div>
    );
  }
      ...
      <div className="p-6">
        <Card className="border-red-200 bg-red-50">
          <CardContent className="pt-6">
            <p className="text-red-600">Failed to load tail spend data. Please try again later.</p>
          </CardContent>
        </Card>
      </div>
    );
  ...
  const { summary, segments, pareto_data, category_analysis, consolidation_opportunities } = data;

  // Prepare Pareto chart data with cumulative percentage
  const paretoChartData = pareto_data.map(item => ({
    supplier: item.supplier.length > 15 ? item.supplier.substring(0, 15) + '...' : item.supplier,
        ...
        <CardContent className="pt-6">
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1">
              <h3 className="font-semibold text-blue-900 dark:text-blue-100 mb-1">What is Tail Spend?</h3>
              <p className="text-sm text-blue-800 dark:text-blue-200">
                <strong>Tail spend</strong> refers to suppliers with annual spend below the threshold.
              ...
              <div className="flex items-center gap-4">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <Input
                    type="number"
                    value={threshold}
                    onChange={(e) => setThreshold(Math.max(10000, Math.min(100000, parseInt(e.target.value) || 50000)))}
                    className="w-24"
                  />
                </div>
              </div>
              <p className="text-xs text-muted-foreground mt-1">
                Vendors with annual spend below this threshold are classified as tail vendors
              </p>
            </div>
          </div>
        </CardContent>
      ...
      <div className="grid grid-cols-5 gap-4">
        <Card>
          <CardContent className="pt-6 text-center">
            <div className="text-3xl font-bold text-blue-600">{summary.total_vendors}</div>
            <div className="text-sm text-muted-foreground mt-1">Total Vendors</div>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="pt-6 text-center">
            <div className="text-3xl font-bold text-purple-600">{summary.tail_vendor_count}</div>
            <div className="text-sm text-muted-foreground mt-1">Tail Vendors ({summary.vendor_ratio.toFixed(0)}%)</div>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="pt-6 text-center">
        ...
        <Card>
          <CardContent className="pt-6 text-center">
            ...
            <div className="text-sm text-muted-foreground mt-1">% of Total Spend</div>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="pt-6 text-center">
            ...
            <div className="text-sm text-muted-foreground mt-1">Savings Opportunity</div>
          </CardContent>
        </Card>
      </div>
                  ...
                  if (active && payload && payload.length > 0) {
                    ...
                    return (
                      <div className="bg-white dark:bg-gray-800 p-3 border rounded shadow-lg">
                        ...
                        <p className="text-sm">Spend: {formatCurrency(item.spend)}</p>
                        <p className="text-sm">Cumulative: {item.cumulative.toFixed(1)}%</p>
                        <p className={`text-sm font-medium ${item.is_tail ? 'text-red-600' : 'text-blue-600'}`}>
        ...
        <Card className="bg-gradient-to-r from-purple-50 to-blue-50 dark:from-purple-950 dark:to-blue-950 border-purple-200 dark:border-purple-800">
          <CardContent className="pt-6">
            <div className="flex items-center gap-2 mb-4">
              ...
              <h3 className="font-semibold text-purple-900 dark:text-purple-100">Tail Vendor Segmentation Definitions</h3>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <span className="text-sm text-muted-foreground ml-1">$10,000 - {formatCurrency(threshold)} annual spend</span>
                </div>
              </div>
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <span className="text-sm text-muted-foreground ml-1">&gt; {formatCurrency(threshold)} annual spend</span>
                </div>
              </div>
            </div>
          </CardContent>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <Card className="border-l-4 border-l-red-500">
            ...
            <CardContent className="space-y-3">
              <div>
                <div className="text-2xl font-bold">{segments.micro.count} Vendors</div>
                <div className="text-sm text-muted-foreground">Total Vendors</div>
              </div>
              <div>
                <div className="text-lg font-semibold">{formatCurrency(segments.micro.spend)}</div>
                <div className="text-sm text-muted-foreground">Total Spend</div>
              </div>
              <div>
                <div className="text-lg font-semibold">{segments.micro.transactions.toLocaleString()}</div>
          ...
          <Card className="border-l-4 border-l-yellow-500">
            <CardHeader>
              <CardTitle className="flex items-center justify-between">
                ...
                <span className="text-sm text-muted-foreground">$10K - {formatCurrency(threshold)}</span>
              </CardTitle>
            </CardHeader>
            <CardContent className="space-y-3">
              <div>
                <div className="text-2xl font-bold">{segments.small.count} Vendors</div>
                <div className="text-sm text-muted-foreground">Total Vendors</div>
              </div>
              <div>
                <div className="text-lg font-semibold">{formatCurrency(segments.small.spend)}</div>
                <div className="text-sm text-muted-foreground">Total Spend</div>
              </div>
              <div>
                <div className="text-lg font-semibold">{segments.small.transactions.toLocaleString()}</div>
                <div className="text-sm text-muted-foreground">Transactions</div>
              </div>
          ...
          <Card className="border-l-4 border-l-green-500">
            <CardHeader>
              <CardTitle className="flex items-center justify-between">
                ...
                <span className="text-sm text-muted-foreground">&gt; {formatCurrency(threshold)}</span>
              </CardTitle>
            </CardHeader>
            <CardContent className="space-y-3">
              <div>
                <div className="text-2xl font-bold">{segments.non_tail.count} Vendors</div>
                <div className="text-sm text-muted-foreground">Total Vendors</div>
              </div>
              <div>
                <div className="text-lg font-semibold">{formatCurrency(segments.non_tail.spend)}</div>
                <div className="text-sm text-muted-foreground">Total Spend</div>
              </div>
              <div>
                <div className="text-lg font-semibold">{segments.non_tail.transactions.toLocaleString()}</div>
                <div className="text-sm text-muted-foreground">Transactions</div>
              </div>
            ...
            <div className="border-l-4 border-l-green-500 pl-4 py-2">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div className="bg-green-50 dark:bg-green-950 p-3 rounded-lg">
                  <div className="text-sm text-muted-foreground">Optimistic Savings</div>
                  <div className="text-xl font-bold text-green-600">
                    {formatCurrency(summary.tail_spend * 0.10)}
                  </div>
                </div>
                <div className="bg-green-50 dark:bg-green-950 p-3 rounded-lg">
                  <div className="text-sm text-muted-foreground">Vendors Affected</div>
          ...
          <CardContent className="space-y-6">
            ...
            <div className="border-l-4 border-l-green-500 bg-green-50 dark:bg-green-950 p-4 rounded-r-lg">
              <div className="flex items-center justify-between mb-3">
                <h3 className="font-semibold text-lg text-green-700 dark:text-green-300">Phase 1: Quick Wins</h3>
                <span className="text-sm font-medium text-green-600 dark:text-green-400 bg-green-100 dark:bg-green-900 px-3 py-1 rounded-full">0-3 months</span>
              </div>
            ...
            <div className="border-l-4 border-l-blue-500 bg-blue-50 dark:bg-blue-950 p-4 rounded-r-lg">
              <div className="flex items-center justify-between mb-3">
                <h3 className="font-semibold text-lg text-blue-700 dark:text-blue-300">Phase 2: Strategic Implementation</h3>
                <span className="text-sm font-medium text-blue-600 dark:text-blue-400 bg-blue-100 dark:bg-blue-900 px-3 py-1 rounded-full">3-6 months</span>
              </div>
              <ul className="space-y-2 text-sm">
                <li className="flex items-start gap-2">
            ...
            <div className="border-l-4 border-l-purple-500 bg-purple-50 dark:bg-purple-950 p-4 rounded-r-lg">
              <div className="flex items-center justify-between mb-3">
                <h3 className="font-semibold text-lg text-purple-700 dark:text-purple-300">Phase 3: Optimization</h3>
                <span className="text-sm font-medium text-purple-600 dark:text-purple-400 bg-purple-100 dark:bg-purple-900 px-3 py-1 rounded-full">6-12 months</span>
              </div>
              <ul className="space-y-2 text-sm">
                <li className="flex items-start gap-2">
            ...
            <div className="space-y-4">
              {category_analysis.slice(0, 10).map((cat, index) => (
                <div
                  ...
                  onClick={() => cat.category_id && setSelectedCategoryId(cat.category_id)}
                >
                  <h3 className="font-semibold text-lg mb-2">{cat.category}</h3>
                  <div className="grid grid-cols-4 gap-4 text-sm">
                    <div>
                      <span className="font-medium">{cat.tail_percentage.toFixed(1)}%</span>
                      <span className="text-muted-foreground ml-1">Tail %</span>
                    </div>
                    <div>
                      <span className="font-medium">{cat.vendor_percentage.toFixed(0)}%</span>
                      <span className="text-muted-foreground ml-1">Vendor %</span>
                    </div>
                    <div>
                      <span className="font-medium">{formatCurrency(cat.tail_spend)}</span>
                      <span className="text-muted-foreground ml-1">Tail Spend</span>
                    </div>
                    <div>
                      <span className="font-medium">{cat.tail_vendors}</span>
                      <span className="text-muted-foreground ml-1">Tail Vendors</span>
                    </div>
                  </div>
                </div>
              ))}
          ...
          <CardContent>
            ...
            <div className="grid grid-cols-3 gap-4 mb-6">
              <div className="bg-blue-50 dark:bg-blue-950 border border-blue-200 dark:border-blue-800 rounded-lg p-4">
                <div className="text-sm text-blue-600 dark:text-blue-400 font-medium mb-1">Total Opportunities</div>
                <div className="text-2xl font-bold text-blue-900 dark:text-blue-100">{consolidation_opportunities.total_opportunities}</div>
              </div>
              <div className="bg-green-50 dark:bg-green-950 border border-green-200 dark:border-green-800 rounded-lg p-4">
                <div className="text-sm text-green-600 dark:text-green-400 font-medium mb-1">Total Savings Potential</div>
                <div className="text-2xl font-bold text-green-900 dark:text-green-100">{formatCurrency(consolidation_opportunities.total_savings)}</div>
              </div>
              <div className="bg-purple-50 dark:bg-purple-950 border border-purple-200 dark:border-purple-800 rounded-lg p-4">
                <div className="text-sm text-purple-600 dark:text-purple-400 font-medium mb-1">Top Opportunity Type</div>
                <div className="text-2xl font-bold text-purple-900 dark:text-purple-100">{consolidation_opportunities.top_type}</div>
              </div>
            </div>
                ...
                <div>
                  {consolidation_opportunities.multi_category.length === 0 ? (
                    ...
                    <div className="space-y-3">
                      {consolidation_opportunities.multi_category.map((opp, index) => (
                        <div
                          key={index}
                        ...
                        >
                          <div className="flex justify-between items-start mb-2">
                            <h3 className="font-semibold text-lg text-indigo-900 dark:text-indigo-100">{opp.supplier}</h3>
                          ...
                          <div className="grid grid-cols-2 gap-4 text-sm mb-2">
                            <div>
                              <span className="font-medium">Total Spend:</span> {formatCurrency(opp.total_spend)}
                            </div>
                            <div>
                              <span className="font-medium">Savings Potential:</span> {formatCurrency(opp.savings_potential)}
                            </div>
                          </div>
                  ...
                  {consolidation_opportunities.category.length === 0 ? (
                    ...
                    consolidation_opportunities.category.map((opp, index) => (
                      <div
                        ...
                        onClick={() => opp.category_id && setSelectedCategoryId(opp.category_id)}
                      >
                        <h3 className="font-semibold text-lg text-blue-900 dark:text-blue-100 mb-2">{opp.category}</h3>
                        <div className="grid grid-cols-2 gap-4 text-sm mb-3">
                          <div>
                            <span className="font-medium">Total Vendors:</span> {opp.total_vendors}
                          </div>
                          <div>
                            <span className="font-medium">Tail Vendors:</span> {opp.tail_vendors}
                          </div>
                          <div>
                            <span className="font-medium">Tail Spend:</span> {formatCurrency(opp.tail_spend)}
                          </div>
                          <div>
                            <span className="font-medium">Savings Potential:</span> {formatCurrency(opp.savings_potential)}
                          </div>
                        ...
                        <div className="text-sm text-muted-foreground">
                          <span className="font-medium">Top Vendor:</span> {opp.top_vendor}
                        </div>
                        <div className="mt-2 bg-yellow-50 dark:bg-yellow-950 border border-yellow-200 dark:border-yellow-800 rounded p-2 flex items-start gap-2">
                          ...
                          <p className="text-xs text-yellow-800 dark:text-yellow-200">
                            <strong>Recommendation:</strong> Consolidate {opp.tail_vendors} tail vendors to reduce administrative overhead and negotiate better rates.
                          </p>
                        </div>
                      </div>
                    ))
              ...
              {activeTab === 'geographic' && (
                <div className="space-y-3">
                  {consolidation_opportunities.geographic.length === 0 ? (
                    <div className="text-center text-muted-foreground py-8">
                      <p>No geographic consolidation opportunities identified.</p>
                    </div>
                  ) : (
                    consolidation_opportunities.geographic.map((opp, index) => (
                      <div key={index} className="border-l-4 border-l-green-500 bg-green-50 dark:bg-green-950 p-4 rounded-r-lg">
                        <h3 className="font-semibold text-lg text-green-900 dark:text-green-100 mb-2">{opp.location}</h3>
                        <div className="grid grid-cols-2 gap-4 text-sm mb-3">
                          <div>
                            <span className="font-medium">Total Vendors:</span> {opp.total_vendors}
                          </div>
                        ...
                        <div className="mt-2 bg-yellow-50 dark:bg-yellow-950 border border-yellow-200 dark:border-yellow-800 rounded p-2 flex items-start gap-2">
                          <Lightbulb className="h-4 w-4 text-yellow-600 dark:text-yellow-400 mt-0.5" />
                          <p className="text-xs text-yellow-800 dark:text-yellow-200">
                            <strong>Recommendation:</strong> Consolidate {opp.tail_vendors} tail vendors in {opp.location} to leverage regional purchasing power.
                          </p>
                        </div>
                      </div>
                    ))
            ...
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-3 gap-4">
                <div className="bg-gray-50 dark:bg-gray-900 p-4 rounded-lg">
                  <div className="text-sm text-muted-foreground">Total Spend</div>
                  <div className="text-xl font-bold">{formatCurrency(categoryDrilldown.total_spend)}</div>
                </div>
                <div className="bg-red-50 dark:bg-red-950 p-4 rounded-lg">
                  <div className="text-sm text-red-600 dark:text-red-400">Tail Spend</div>
                  <div className="text-xl font-bold text-red-700 dark:text-red-300">{formatCurrency(categoryDrilldown.tail_spend)}</div>
                </div>
                <div className="bg-orange-50 dark:bg-orange-950 p-4 rounded-lg">
                  <div className="text-sm text-orange-600 dark:text-orange-400">Tail %</div>
                  <div className="text-xl font-bold text-orange-700 dark:text-orange-300">{categoryDrilldown.tail_percentage.toFixed(1)}%</div>
                </div>
              </div>
                    ...
                    <tbody>
                      {categoryDrilldown.vendors.map((vendor, idx) => (
                        ...
                        >
                          ...
                          <td className="p-3 text-right">{formatCurrency(vendor.spend)}</td>
                          <td className="p-3 text-right">{vendor.transaction_count}</td>
                          <td className="p-3 text-right">{vendor.percent_of_category.toFixed(1)}%</td>
          ...
          {vendorLoading ? (
            ...
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-4 gap-4">
                <div className="bg-gray-50 dark:bg-gray-900 p-4 rounded-lg">
                  <div className="text-sm text-muted-foreground">Total Spend</div>
                  <div className="text-xl font-bold">{formatCurrency(vendorDrilldown.total_spend)}</div>
                </div>
                <div className="bg-gray-50 dark:bg-gray-900 p-4 rounded-lg">
                  <div className="text-sm text-muted-foreground">Transactions</div>
                  <div className="text-xl font-bold">{vendorDrilldown.transaction_count}</div>
                </div>
                <div className="bg-gray-50 dark:bg-gray-900 p-4 rounded-lg">
                  <div className="text-sm text-muted-foreground">Categories</div>
                  <div className="text-xl font-bold">{vendorDrilldown.categories.length}</div>
                </div>
                <div className={`p-4 rounded-lg ${vendorDrilldown.is_tail ? 'bg-red-50 dark:bg-red-950' : 'bg-green-50 dark:bg-green-950'}`}>
                  <div className={`text-sm ${vendorDrilldown.is_tail ? 'text-red-600 dark:text-red-400' : 'text-green-600 dark:text-green-400'}`}>Status</div>
                  <div className={`text-xl font-bold ${vendorDrilldown.is_tail ? 'text-red-700 dark:text-red-300' : 'text-green-700 dark:text-green-300'}`}>
                    ...
                    <tbody>
                      {vendorDrilldown.categories.map((cat, idx) => (
                        <tr key={idx} className="border-t dark:border-gray-700">
                          ...
                          <td className="p-3 text-right">{formatCurrency(cat.spend)}</td>
                          <td className="p-3 text-right">{cat.transaction_count}</td>
                          <td className="p-3 text-right">{cat.percent_of_vendor.toFixed(1)}%</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
                      ...
                      <tbody>
                        {vendorDrilldown.locations.map((loc, idx) => (
                          <tr key={idx} className="border-t dark:border-gray-700">
                            ...
                            <td className="p-3 text-right">{formatCurrency(loc.spend)}</td>
                            <td className="p-3 text-right">{loc.transaction_count}</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
