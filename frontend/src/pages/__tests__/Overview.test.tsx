...
describe('Overview Page', () => {
  beforeEach(() => {
    ...
    vi.mocked(useFilteredProcurementData).mockReturnValue({
      ...
      isError: false,
      isSuccess: true,
    } as ReturnType<typeof useFilteredProcurementData>);
  ...
  describe('Loading State', () => {
    ...
    it('should display page header even during loading', () => {
      ...
      expect(screen.getByText('Key metrics and insights from your procurement data')).toBeInTheDocument();
    });
  });
  ...
  describe('Empty State', () => {
    ...
    it('should show admin upload link for admin users', () => {
      ...
      expect(screen.getByText('Upload your procurement data via the Admin Panel to see analytics and insights.')).toBeInTheDocument();
    ...
    it('should show different message for non-admin users', () => {
      ...
      expect(screen.getByText('Contact an administrator to upload procurement data to see analytics and insights.')).toBeInTheDocument();
  ...
  describe('Charts Display', () => {
    it('should display all four charts', () => {
      ...
      expect(screen.getByTestId('chart-spend-trend-over-time')).toBeInTheDocument();
      expect(screen.getByTestId('chart-top-10-suppliers')).toBeInTheDocument();
      expect(screen.getByTestId('chart-spend-distribution')).toBeInTheDocument();
    });

    it('should display chart descriptions', () => {
      render(<Overview />, { wrapper: createWrapper() });

      expect(screen.getByText(/Shows how your procurement budget is distributed/)).toBeInTheDocument();
      expect(screen.getByText(/Track monthly spending patterns/)).toBeInTheDocument();
      expect(screen.getByText(/Your largest vendors by total spend/)).toBeInTheDocument();
      expect(screen.getByText(/Categorizes transactions into High.*Medium.*and Low/)).toBeInTheDocument();
    });
  });
