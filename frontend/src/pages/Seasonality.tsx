...
import { useDetailedSeasonality, useSeasonalityCategoryDrilldown } from '@/hooks/useAnalytics';
...
function getHeatmapColor(index: number): string {
  ...
  if (index < 85) return 'bg-blue-300 text-blue-900 dark:bg-blue-700 dark:text-blue-100';
  if (index < 95) return 'bg-blue-100 text-blue-800 dark:bg-blue-900/50 dark:text-blue-200';
  if (index <= 105) return 'bg-gray-100 text-gray-800 dark:bg-gray-700 dark:text-gray-200';
  if (index <= 115) return 'bg-orange-100 text-orange-800 dark:bg-orange-900/50 dark:text-orange-200';
  if (index <= 130) return 'bg-orange-300 text-orange-900 dark:bg-orange-700 dark:text-orange-100';
    ...
    <Card>
      ...
      <CardContent>
        <p className="text-sm text-gray-600 dark:text-gray-400 mb-4">
          Click any row to see supplier details for that category. Values show seasonal index (100 = average).
        </p>
          ...
          <table className="w-full text-xs border-collapse">
            <thead>
              <tr className="border-b dark:border-gray-700">
                ...
                {monthNames.map(month => (
                  <th key={month} className="p-2 text-center font-semibold min-w-[50px]">
                    {month}
                  </th>
                ))}
              </tr>
            ...
            <tbody>
              {categories.map(cat => (
                ...
                >
                  <td className="p-2 font-medium sticky left-0 bg-white dark:bg-gray-900 truncate max-w-[150px]" title={cat.category}>
                    {cat.category}
                  </td>
        ...
        <div className="flex items-center justify-center gap-4 mt-4 text-xs flex-wrap">
          ...
          <span className="flex items-center gap-1">
            <div className="w-4 h-4 bg-blue-100 dark:bg-blue-900/50 border rounded" /> Below Avg
          </span>
          <span className="flex items-center gap-1">
            <div className="w-4 h-4 bg-gray-100 dark:bg-gray-700 border rounded" /> Average (100)
          </span>
          <span className="flex items-center gap-1">
            <div className="w-4 h-4 bg-orange-100 dark:bg-orange-900/50 border rounded" /> Above Avg
          </span>
          <span className="flex items-center gap-1">
...
export default function Seasonality() {
  ...
  const { data: drilldownData, isLoading: drilldownLoading } = useSeasonalityCategoryDrilldown(
    selectedCategoryId,
    useFiscalYear
  );
          ...
          <div className="grid grid-cols-4 gap-4 mb-6">
            <Card className="bg-white dark:bg-gray-800">
              <CardContent className="pt-6 text-center">
                ...
                <div className="text-sm text-gray-600 dark:text-gray-400 mt-1">Categories Analyzed</div>
              </CardContent>
            </Card>
            ...
            <Card className="bg-white dark:bg-gray-800">
              <CardContent className="pt-6 text-center">
                ...
                <div className="text-sm text-gray-600 dark:text-gray-400 mt-1">Opportunities Found</div>
              </CardContent>
            </Card>
            ...
            <Card className="bg-white dark:bg-gray-800">
              <CardContent className="pt-6 text-center">
                ...
                <div className="text-sm text-gray-600 dark:text-gray-400 mt-1">High Impact</div>
              </CardContent>
            </Card>
            ...
            <Card className="bg-white dark:bg-gray-800">
              <CardContent className="pt-6 text-center">
                <div className="text-3xl font-bold text-cyan-600 dark:text-cyan-400">
                  ${(summary?.total_savings_potential ?? 0).toLocaleString(undefined, { minimumFractionDigits: 0, maximumFractionDigits: 0 })}
                </div>
                <div className="text-sm text-gray-600 dark:text-gray-400 mt-1">Savings Potential</div>
              </CardContent>
            </Card>
          </div>
          ...
          <Card className="bg-white dark:bg-gray-800">
            <CardContent className="pt-4">
              <div className="flex items-start gap-2">
                ...
                <div>
                  <span className="font-semibold text-gray-800 dark:text-gray-200">Key Insights</span>
                  <p className="text-sm text-gray-700 dark:text-gray-300 mt-1">
                    Analysis of {summary?.categories_analyzed ?? 0} categories revealed {summary?.opportunities_found ?? 0} optimization opportunities with potential savings of ${(summary?.total_savings_potential ?? 0).toLocaleString()}. {summary?.high_impact_count ?? 0} high-impact opportunities identified for immediate action.
                  </p>
                </div>
              </div>
            </CardContent>
      ...
      <div className="flex items-center justify-between flex-wrap gap-4">
        ...
        <div className="flex items-center gap-4">
          <div className="text-sm text-gray-600 dark:text-gray-400">
            ...
            {availableYears.length >= 2 && (
              <>
                ...
                <span className={(summary?.avg_yoy_growth ?? 0) >= 0 ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'}>
                  {(summary?.avg_yoy_growth ?? 0) >= 0 ? '+' : ''}{(summary?.avg_yoy_growth ?? 0).toFixed(1)}%
                </span>
              </>
            )}
          </div>
          ...
          <div className="flex items-center gap-2">
            ...
            />
            <label htmlFor="fiscal-year" className="text-sm font-medium text-blue-600 dark:text-blue-400 cursor-pointer">
            ...
            <CardContent className="pt-6">
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1">
                  <h3 className="text-sm font-medium text-gray-600 dark:text-gray-400 mb-1">Highest Seasonality</h3>
                  <p className="text-lg font-bold text-gray-900 dark:text-gray-100">{categorySeasonality[0].category}</p>
                  <div className="mt-2 space-y-1">
                    <p className="text-sm text-gray-600 dark:text-gray-400">
                      <span className="font-medium">Strength:</span> {categorySeasonality[0].seasonality_strength.toFixed(1)}%
                    </p>
                    <p className="text-sm text-gray-600 dark:text-gray-400">
                      <span className="font-medium">Peak:</span> {categorySeasonality[0].peak_month} |
                      <span className="font-medium"> Low:</span> {categorySeasonality[0].low_month}
                    </p>
                  </div>
                </div>
              </div>
          ...
          <Card className="border-l-4 border-l-green-500">
            <CardContent className="pt-6">
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1">
                  <h3 className="text-sm font-medium text-gray-600 dark:text-gray-400 mb-1">Lowest Seasonality</h3>
                  <p className="text-lg font-bold text-gray-900 dark:text-gray-100">{categorySeasonality[categorySeasonality.length - 1].category}</p>
                  <div className="mt-2 space-y-1">
                    <p className="text-sm text-gray-600 dark:text-gray-400">
                      <span className="font-medium">Strength:</span> {categorySeasonality[categorySeasonality.length - 1].seasonality_strength.toFixed(1)}%
                    </p>
                    <p className="text-sm text-gray-600 dark:text-gray-400">
                      <span className="font-medium">Peak:</span> {categorySeasonality[categorySeasonality.length - 1].peak_month} |
                      <span className="font-medium"> Low:</span> {categorySeasonality[categorySeasonality.length - 1].low_month}
                    </p>
                  </div>
                </div>
              </div>
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-gray-600 dark:text-gray-400 mt-1">
                      Peak spending in {category.peak_month}, low in {category.low_month}. Seasonality strength: {category.seasonality_strength.toFixed(1)}%
                    </p>

                    {/* YoY Growth Badge */}
                    {recentYears.length >= 2 && prevFY && currFY && (
                      <div className="flex items-center gap-2 mt-2">
                        <span className="text-sm text-gray-600 dark:text-gray-400">YoY Growth ({prevFY} → {currFY}):</span>
                        <Badge variant={category.yoy_growth >= 0 ? 'default' : 'destructive'} className="gap-1">
                          {category.yoy_growth >= 0 ? (
                            <TrendingUp className="h-3 w-3" />
                          ) : (
                            <TrendingDown className="h-3 w-3" />
                          )}
                          {category.yoy_growth >= 0 ? '+' : ''}{category.yoy_growth.toFixed(1)}%
                        </Badge>
                        <span className="text-sm text-gray-500 dark:text-gray-500">
                          ${(category.fy_totals[prevFY] ?? 0).toLocaleString()} → ${(category.fy_totals[currFY] ?? 0).toLocaleString()}
                        </span>
                      </div>
                    )}
                  </div>
                  ...
                  <div className="grid grid-cols-3 gap-4">
                    <Card className="bg-cyan-50 dark:bg-cyan-900/30">
                      <CardContent className="pt-4 text-center">
                        <div className="text-2xl font-bold text-cyan-700 dark:text-cyan-400">
                          ${category.savings_potential.toLocaleString(undefined, { minimumFractionDigits: 0, maximumFractionDigits: 0 })}
                        </div>
                        <div className="text-sm text-gray-600 dark:text-gray-400 mt-1">Savings Potential</div>
                      </CardContent>
                    </Card>
                    ...
                    <Card className="bg-cyan-50 dark:bg-cyan-900/30">
                      <CardContent className="pt-4 text-center">
                        <div className="text-2xl font-bold text-cyan-700 dark:text-cyan-400">{category.impact_level}</div>
                        <div className="text-sm text-gray-600 dark:text-gray-400 mt-1">Impact Level</div>
                      </CardContent>
                    </Card>

                    <Card className="bg-cyan-50 dark:bg-cyan-900/30">
                      <CardContent className="pt-4 text-center">
                        <div className="text-2xl font-bold text-cyan-700 dark:text-cyan-400">6-12 months</div>
                        <div className="text-sm text-gray-600 dark:text-gray-400 mt-1">Timeline</div>
                      </CardContent>
                    </Card>
                  </div>
                  ...
                  <div className="bg-yellow-50 dark:bg-yellow-900/30 border-l-4 border-l-yellow-400 p-4 rounded">
                    <div className="flex items-start gap-2">
                      ...
                      <p className="text-sm text-gray-700 dark:text-gray-300">
                        Contract during {category.low_month} (low demand) for {category.peak_month} (peak demand) services to optimize costs.
                      </p>
                    </div>
                  </div>
                </div>
        ...
        <DialogContent size="xl" className="max-h-[90vh] overflow-y-auto dark:bg-gray-800">
          ...
          {drilldownLoading ? (
            ...
            <div className="space-y-6 py-4">
                ...
                <Card className="bg-purple-50 dark:bg-purple-900/30">
                  <CardContent className="pt-4 text-center">
                    ...
                    <div className="text-xl font-bold text-purple-700 dark:text-purple-400">
                      ${Math.round(drilldownData.total_spend / drilldownData.supplier_count).toLocaleString()}
                    </div>
                    <div className="text-sm text-gray-600 dark:text-gray-400">Avg/Supplier</div>
                  </CardContent>
                </Card>
              </div>
              ...
              <Card>
                <CardHeader>
                  <CardTitle className="text-base">Monthly Spend Distribution</CardTitle>
                </CardHeader>
                <CardContent>
                  ...
                  <div className="overflow-x-auto">
                    <table className="w-full text-sm">
                      <thead>
                        <tr className="border-b dark:border-gray-700">
                          ...
                          <th className="text-right p-2 font-semibold">% of Category</th>
                          <th className="text-center p-2 font-semibold">Peak</th>
                          <th className="text-center p-2 font-semibold">Low</th>
                          <th className="text-right p-2 font-semibold">Seasonality</th>
                        </tr>
                      </thead>
                      <tbody>
                        {drilldownData.suppliers.slice(0, 10).map((supplier, idx) => (
                          <tr key={idx} className="border-b dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-800">
                            ...
                            <td className="p-2 text-right">${supplier.total_spend.toLocaleString()}</td>
                            <td className="p-2 text-right">{supplier.percent_of_category.toFixed(1)}%</td>
                            ...
                            <td className="p-2 text-right">
                              <span className={supplier.seasonality_strength > 30 ? 'text-red-600 font-medium' : supplier.seasonality_strength > 20 ? 'text-orange-600' : 'text-green-600'}>
                    ...
                    {drilldownData.suppliers.length > 10 && (
                      <p className="text-sm text-gray-500 dark:text-gray-400 mt-2 text-center">
                        Showing top 10 of {drilldownData.suppliers.length} suppliers
                      </p>
                    )}
                  </div>
                </CardContent>
