...
async function navigateToDataUploadList(page: Page) {
  ...
  await expect(page.locator('h1')).toContainText(/Data upload|Select data upload/i);
}
...
test.describe('Upload Wizard - File Upload Flow', () => {
  ...
  test('should accept CSV file', async ({ page }) => {
    ...
    await expect(page.locator('.file-info, .file-name, text=test-upload.csv')).toBeVisible({ timeout: 5000 });
  });
  ...
  test('should reject non-CSV file', async ({ page }) => {
    ...
    try {
      ...
      await expect(page.locator('.error, .alert-danger, text=/CSV/i')).toBeVisible({ timeout: 5000 });
    } finally {
  ...
  test('should proceed to step 2 after file selection', async ({ page }) => {
    ...
    if (await nextButton.isVisible()) {
      ...
      await expect(page.locator('#step-2, [data-step="2"]')).toBeVisible({ timeout: 10000 });
    }
  });
});
...
test.describe('Upload Wizard - Preview Step', () => {
  ...
  test('should display data preview table', async ({ page }) => {
    ...
    const previewTable = page.locator('table.preview-table, #preview-table, [data-preview]');
  ...
  test('should display file metadata', async ({ page }) => {
    ...
    await expect(page.locator('text=/rows|records/i')).toBeVisible({ timeout: 10000 });
  });
});
...
test.describe('Upload Wizard - Column Mapping Step', () => {
  ...
  test('should display column mapping interface', async ({ page }) => {
    ...
    const mappingSection = page.locator('#step-3, [data-step="3"], .mapping-container');
  ...
  test('should auto-detect column mappings', async ({ page }) => {
    ...
    const supplierMapping = page.locator('select:has(option[value="supplier"]:checked), [data-mapping="supplier"]');

    // The mapping should be detected (our CSV has matching column names)
    await expect(page.locator('text=/mapped|detected/i')).toBeVisible({ timeout: 5000 });
  });

  test('should allow saving mapping template', async ({ page }) => {
    // Should have save template button
    const saveButton = page.locator('button:has-text("Save"), button:has-text("Template")');
...
test.describe('Upload Wizard - Validation Step', () => {
  ...
  test('should display validation results', async ({ page }) => {
    const validationSection = page.locator('#step-4, [data-step="4"], .validation-container');
  ...
  test('should show skip invalid option', async ({ page }) => {
    ...
    const skipOption = page.locator('input[type="checkbox"]:near(:text("skip")), label:has-text("skip")');
...
test.describe('Upload Wizard - Upload Progress Step', () => {
  test.beforeEach(async ({ page }) => {
    ...
    for (let i = 0; i < 4; i++) {
      const nextButton = page.locator('button:has-text("Next"), .btn-next, button:has-text("Upload"), button:has-text("Start")');
      if (await nextButton.isVisible()) {
        await nextButton.click();
        await page.waitForTimeout(2000);
      }
  ...
  test('should display upload progress', async ({ page }) => {
    const progressSection = page.locator('#step-5, [data-step="5"], .upload-progress');
    await expect(progressSection).toBeVisible({ timeout: 15000 });

    // Should show progress indicator
    await expect(page.locator('.progress-bar, progress, [role="progressbar"]')).toBeVisible();
  });
  ...
  test('should show completion status', async ({ page }) => {
    ...
    await expect(page.locator('text=/complete|success|finished/i')).toBeVisible({ timeout: 30000 });
  });
});
...
test.describe('Organization Management - Delete All Data', () => {
  ...
  test('should not delete without correct confirmation', async ({ page }) => {
    ...
    // Should show error
    await expect(page.locator('text=/error|incorrect|match/i')).toBeVisible({ timeout: 5000 });
  });
});
...
test.describe('Organization Management - Reset Organization', () => {
  ...
  test('should show Reset Organization button for superuser', async ({ page }) => {
...
test.describe('Upload History', () => {
  ...
  test('should show status badges', async ({ page }) => {
    ...
    await expect(page.locator('th:has-text("Status"), td:has-text("Status")')).toBeVisible();
  });
  ...
  test('should have filter options', async ({ page }) => {
    ...
    if (await filterSection.isVisible()) {
      ...
      await expect(filterSection.locator('text=/date|status|organization/i')).toBeVisible();
    }
  });
...
test.describe('Column Mapping Templates', () => {
  ...
  test('should display template management UI', async ({ page }) => {
    ...
    await expect(page.locator('text=/template/i')).toBeVisible({ timeout: 10000 });
  });

  test('should allow loading existing template', async ({ page }) => {
    // Should have template dropdown or list
    const templateSelect = page.locator('select:has-text("template"), [data-templates]');
...
test.describe('Error Handling', () => {
  ...
  test('should handle invalid file format', async ({ page }) => {
    ...
    try {
      ...
      await expect(page.locator('text=/invalid|csv|format/i')).toBeVisible({ timeout: 5000 });
    } finally {
